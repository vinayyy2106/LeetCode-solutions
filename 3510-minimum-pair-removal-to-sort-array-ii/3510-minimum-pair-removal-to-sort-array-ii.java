class Solution {
    ...
    public int minimumPairRemoval(int[] nums) {
        ...
        int ops=0;
        while(badPairs>0){
            Pair curr=minPairSet.first();
            minPairSet.remove(curr);
            int first = curr.idx;
            ...
            if(first_left>=0){
                minPairSet.remove(new Pair(temp[first_left] + temp[first], first_left));
                minPairSet.add(new Pair(temp[first_left] + temp[first] + temp[second], first_left));
            }
            if(second_right<n){
                minPairSet.remove(new Pair(temp[second]+temp[second_right],second));
                minPairSet.add(new Pair(temp[first] + temp[second] + temp[second_right], first));
